...
<div class="card">
    <div class="card-body">
        <div class="d-flex align-content-center justify-content-between mb-3">
            ...
            <a href="/device/create" class="btn btn-primary">
        ...
        <table class="table table-striped table-bordered">
            <thead>
              <tr>
                <th scope="col">#</th>
                <th scope="col">
    <a href="?sort_name=device_id&sort_val={{$sort}}">Device ID
        @if (request('sort_name')=='device_id' && request('sort_val')=='ASC')
...
<th scope="col">
    <a href="?sort_name=phone&sort_val={{$sort}}">No WA
...
<td>{{$item->status}}</td>
                    ...
                    <td>
                        <a class="btn btn-warning btn-sm" href="/device/{{$item->id}}">
                            <span data-feather="eye"></span>
                        </a>
                        <a class="btn btn-primary btn-sm" href="/device/{{$item->id}}/edit">
                            <span data-feather="edit"></span>
                        </a>

                        <a class="btn btn-danger btn-sm" href="javascript:" onclick="deleteData({{$item->id}})"><span data-feather="trash"></span></a>
                        <form class="d-none" id="formdelete-{{$item->id}}" action="/device/{{$item->id}}" method="POST">
                            @method('DELETE')
                            @csrf
                        </form>
                    </td>
